<x-admin.layout :title="'Ficha Técnica — ' . $property->title" :breadcrumb="'Imóveis / Ficha Técnica #' . str_pad($property->id, 4, '0', STR_PAD_LEFT)">
    ...
    <div class="space-y-6 max-w-6xl mx-auto pb-12" x-data="{ selectedImg: '{{ $property->image ? (file_exists(public_path('assets/' . $property->image)) ? asset('assets/' . $property->image) : (str_starts_with($property->image, 'properties/') ? Storage::url($property->image) : asset('assets/' . $property->image))) : '' }}' }">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 no-print">
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.properties.index') }}"
                   class="w-10 h-10 rounded-xl bg-white border border-admin-border flex items-center justify-center text-admin-muted hover:text-admin-text hover:border-brand transition shadow-sm">
                    <i class="fa-solid fa-arrow-left text-sm"></i>
                </a>
                <div>
                    <h1 class="text-xl font-bold text-gray-900 flex items-center gap-2">
                        Ficha Técnica do Imóvel
                        <span class="text-xs font-mono font-semibold bg-gray-100 text-gray-600 px-2.5 py-0.5 rounded-md border border-gray-200">
            ...
            <div class="flex items-center flex-wrap gap-2.5">
                <button onclick="window.print()"
                        class="bg-white hover:bg-gray-50 border border-gray-200 text-gray-700 text-xs font-bold px-4 py-2.5 rounded-xl shadow-sm flex items-center gap-2 transition">
                ...
                <a href="{{ route('admin.properties.edit', $property) }}"
                   class="bg-brand hover:bg-brand-dark text-white text-xs font-bold px-5 py-2.5 rounded-xl shadow-sm flex items-center gap-2 transition">
        ...
        {{-- Main Overview Card --}}
        <div class="bg-white rounded-2xl border border-admin-border overflow-hidden shadow-sm print-card">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-0">
                ...
                <div class="lg:col-span-5 bg-gray-900 p-6 flex flex-col justify-between relative min-h-[340px]">
                    <div class="relative w-full h-72 rounded-xl overflow-hidden bg-black/40 border border-white/10 flex items-center justify-center">
                    ...
                    <div class="flex items-center gap-2 mt-4 overflow-x-auto pb-1 no-print">
                        ...
                        <button type="button" @click="selectedImg = '{{ $imgSrc }}'"
                                ...
                                class="w-14 h-11 rounded-lg overflow-hidden flex-shrink-0 border border-white/20 transition">
                ...
                <div class="lg:col-span-7 p-6 lg:p-8 flex flex-col justify-between space-y-6">
                    <div>
                        ...
                        {{-- Price Box --}}
                        <div class="mt-5 p-4 rounded-xl bg-orange-50/70 border border-orange-200 flex items-center justify-between">
                            <div>
                            ...
                            <div class="w-11 h-11 rounded-xl bg-brand text-white flex items-center justify-center font-bold text-lg shadow-sm">
        ...
        <div class="bg-amber-50/60 rounded-2xl border border-amber-200 p-6 shadow-sm print-card">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {{-- Owner Name --}}
                <div class="bg-white p-4 rounded-xl border border-amber-200/80 shadow-2xs">
            ...
            <div class="mt-3 bg-white p-3.5 rounded-xl border border-amber-200/80 text-xs flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                {{-- Description Card --}}
                <div class="bg-white rounded-2xl border border-admin-border p-6 shadow-sm print-card">
                    <h3 class="font-bold text-gray-900 text-base mb-4 flex items-center gap-2">
                        <i class="fa-solid fa-align-left text-brand"></i>
                        Descrição Detalhada do Imóvel
                    </h3>
                    <div class="text-sm text-gray-700 leading-relaxed whitespace-pre-line bg-gray-50/50 p-5 rounded-xl border border-gray-100">
                ...
                <div class="bg-white rounded-2xl border border-admin-border p-6 shadow-sm print-card">
                    ...
                    @if(!empty($property->amenities) && is_array($property->amenities))
                        <div class="flex flex-wrap gap-2.5">
                            ...
                            <span class="bg-gray-50 border border-gray-200 text-gray-800 text-xs font-semibold px-3 py-1.5 rounded-lg flex items-center gap-1.5">
            ...
            <div class="space-y-6">
                    ...
                    <dl class="space-y-3 text-sm">
                        ...
                        <div class="pt-2">
                            ...
                            <a href="https://www.google.com/maps/search/?api=1&query={{ $property->latitude }},{{ $property->longitude }}"
                               target="_blank"
                               class="w-full py-2 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-xl text-xs font-bold text-brand flex items-center justify-center gap-1.5 transition">
                ...
                <div class="bg-white rounded-2xl border border-admin-border p-6 shadow-sm print-card no-print">
                    <h3 class="font-bold text-gray-900 text-base mb-4 flex items-center gap-2">
                    ...
                    <div class="space-y-3">
                        ...
                        <a href="{{ $property->video_url }}" target="_blank"
                           class="w-full p-3 bg-red-50 hover:bg-red-100 border border-red-200 rounded-xl text-xs font-bold text-red-600 flex items-center justify-between transition">
                        ...
                        <a href="{{ $property->tour_3d_url }}" target="_blank"
                           class="w-full p-3 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 rounded-xl text-xs font-bold text-indigo-700 flex items-center justify-between transition">
                            <span class="flex items-center gap-2">
        ...
        <div class="bg-white rounded-2xl border border-admin-border p-6 shadow-sm print-card">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                ...
                <button type="button" @click="selectedImg = '{{ $gSrc }}'; window.scrollTo({ top: 0, behavior: 'smooth' })"
                        class="group relative h-28 rounded-xl overflow-hidden bg-gray-100 border border-gray-200 hover:border-brand transition shadow-2xs">
