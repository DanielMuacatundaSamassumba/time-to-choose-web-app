<x-layouts.app :title="$property->title" :description="$property->description">
    ...
    <section class="max-w-7xl mx-auto px-4 lg:px-6 py-8 mt-6">
        ...
        <div class="relative">
            <!-- Slider Principal -->
            <div class="swiper propertyGallery rounded-3xl overflow-hidden shadow-md">
            ...
            <div class="swiper propertyThumbs mt-4">
                <div class="swiper-wrapper">
                    ...
                    @endphp
                    <div class="swiper-slide">
                        <img src="{{ $url }}" class="rounded-xl h-20 w-full object-cover cursor-pointer hover:opacity-85 transition">
                    </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="lg:col-span-2">
                ...
                <section class="mt-10">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                        ...
                        <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 text-gray-700 font-medium flex items-center gap-2">
                ...
                @if($property->tour_3d_url)
                    <section class="mt-10">
                        ...
                        <div class="aspect-video rounded-2xl overflow-hidden shadow-md bg-gray-100 border border-gray-200">
                            <iframe class="w-full h-full border-0" src="{{ $property->tour_3d_url }}" allowfullscreen allow="xr-spatial-tracking"></iframe>
                        </div>
                    </section>
                @endif
                ...
                <section class="mt-10">
                    ...
                    <div class="rounded-2xl h-96 flex items-center justify-center overflow-hidden shadow-md border border-gray-100">
            ...
            <aside>
                <div class="lg:sticky lg:top-24 bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
                    ...
                    <a href="https://wa.me/244923000000?text=Olá,%20tenho%20interesse%20no%20imóvel%20{{ urlencode($property->title) }}"
                       target="_blank"
                       class="w-full border border-green-200 py-4 rounded-xl mt-3 flex items-center justify-center gap-2 font-semibold text-green-700 hover:bg-green-50 transition">
                    ...
                    <div class="mb-4 bg-green-50 border border-green-200 text-green-700 p-4 rounded-xl text-sm font-medium">
                        {{ session('contact_success') }}
                    </div>
                    @endif

                    @if($errors->any())
                    <div class="mb-4 bg-red-50 border border-red-200 text-red-600 p-4 rounded-xl text-xs">
                        {{ $errors->first() }}
                    </div>
                    @endif
    ...
    <section class="max-w-7xl mx-auto px-4 lg:px-6 py-12 border-t border-gray-100">
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <article class="property-card bg-white rounded-[20px] overflow-hidden border border-[#E5E5E5] shadow-lg flex flex-col h-full hover:shadow-2xl transition duration-300">
                <div class="relative overflow-hidden group">
                    @if($item->image && file_exists(public_path('assets/' . $item->image)))
                        <img src="{{ asset('assets/' . $item->image) }}" alt="{{ $item->title }}" class="w-full h-[260px] object-cover group-hover:scale-105 transition duration-500">
                    @elseif($item->image && str_starts_with($item->image, 'properties/'))
                        <img src="{{ Storage::url($item->image) }}" alt="{{ $item->title }}" class="w-full h-[260px] object-cover group-hover:scale-105 transition duration-500">
                    @else
